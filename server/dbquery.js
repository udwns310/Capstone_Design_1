...
exports.login = function (req, res) {
  const post = req.body;
  db.query(
    `SELECT password, salt FROM profile where email = ?`,
    [post.email],
...
exports.register = function (req, res) {
  const post = req.body;
  const hasEmptyValue = Object.values(post).some(
    (value) => value.trim() === ""
  );
  if (hasEmptyValue) {
    ...
    db.query(
      `SELECT * FROM profile where email = ?`, // email이 중복되는지 검사
      [post.email],
      function (Eerr, Erows) {
        if (Eerr) throw Eerr;
        if (Erows.length == 0) {
          // email이 중복되지 않는다면
          db.query(
            `SELECT * FROM profile where stdId = ?`, // stdId가 중복되는지 검사
            [post.stdId],
            async function (Serr, Srows) {
              if (Serr) throw Serr;
              if (Srows.length == 0) {
                // stdId가 중복되지 않는다면
                ...
                db.query(
                  `INSERT INTO profile VALUES (?, ?, ?, ?, ?, ?, NULL, ?)`, // DB에 데이터 삽입
                  [
                    post.email,
                    password,
                    post.name,
